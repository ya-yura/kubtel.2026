...
## Чекап модернизации по brief-ответам 2026-06-11

- [x] Изучены материалы `brief/b2b.txt`, `brief/b2g.txt`, `brief/телефония.txt`, `brief/телевидение.txt`.
- [x] Зафиксировано, что `brief/b2c.txt` присутствует, но пустой.
- [x] `/business/` усилен под фактические B2B JTBD: быстро подключить офис, переехать, уйти от плохого качества, не ошибиться с оплатой.
- [x] Добавлен прямой B2B-контакт `+7 861 200-10-34` и FAQ по подключению, смене тарифа и оплате.
- [x] `/business/b2g/` расширен под 44-ФЗ, 223-ФЗ, ЕИС, федеральные ЭТП, контакты B2G и список обсуждаемых услуг.
- [x] `/business/request/` получил B2G-режим с именем/отчеством, рабочим телефоном, мобильным по желанию, e-mail и предпочтительным каналом связи.
- [x] B2G-лиды маршрутизируются как отдельный pipeline `b2g` с `department=b2g`, `priority=urgent`, `slaResponseMinutes=30`.
- [x] Телефонный калькулятор перевёл часть терминов на клиентский язык и учитывает населённый пункт.
- [x] ТВ-сценарий обновлён как `Интернет + ТВ`, Smart TV/приставка, архив, действующий абонент и поддержка.
- [x] Добавлен документ `docs/jtbd-brief-modernization-2026-06-11.md`.
- [ ] Нужно подтвердить боевую доставку B2G в Redmine/CRM, включая проект «Общий», трекер «Входящее обращение» и назначение на B2G.
- [ ] Нужно подтвердить лицензии, реквизиты, цены, список каналов, ссылки на Smart TV-приложения/APK и production-интеграции.
